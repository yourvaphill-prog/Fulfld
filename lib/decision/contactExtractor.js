...
// Any token matching this set disqualifies a name candidate (checked on all tokens).
const NAME_BLOCKLIST = new Set([
  ...
  'shipping', 'returns', 'return', 'checkout', 'cart', 'order', 'track', 'delivery',
  ...
  // action/navigation words that appear near contact sections but are not name tokens
  'help', 'center', 'email', 'send', 'call', 'policy', 'cookie',
  'status', 'locator', 'exchange', 'exchanges', 'account',
]);
...
/**
 * Returns false for phone numbers that cannot be valid US numbers:
 * area code or exchange starting with 0 or 1, or obvious placeholder segments.
 * Toll-free prefixes (800/833/844/855/866/877/888) are preserved if exchange is valid.
 */
function isValidUsPhone(normalized) {
  if (!normalized) return false;
  const m = normalized.match(/^\((\d{3})\) (\d{3})-(\d{4})$/);
  if (!m) return false;
  const [, area, exchange, line] = m;
  if (area[0] === '0' || area[0] === '1') return false;
  if (exchange[0] === '0' || exchange[0] === '1') return false;
  if (area === '000' || exchange === '000' || line === '0000') return false;
  return true;
}
...
function isValidName(str) {
  ...
  // Reject all-caps phrases (e.g. "SEND US AN EMAIL", "HELP CENTER")
  if (tokens.every(t => t === t.toUpperCase())) return false;
  // Reject if ANY token is a blocklisted non-name word
  const lowerTokens = tokens.map(t => t.toLowerCase());
  if (lowerTokens.some(t => NAME_BLOCKLIST.has(t))) return false;
  return true;
}
...
export function extractContacts(fetchedPages) {
  ...
  for (const page of fetchedPages) {
    ...
    for (const raw of [...extractTelNumbers(html), ...extractTextPhones(htmlNoScripts)]) {
      ...
      if (norm && isValidUsPhone(norm) && !phoneMap.has(norm)) {
